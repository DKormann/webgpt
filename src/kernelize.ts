...
export const mkKernel = (g:UOp):UOp => mkUop("KERNEL", [g], {arg:{size:numel(uop.shape(g))}})

let pm = new PatternMatcher([
  [new UPat("r", "RESHAPE", [new UPat("x")]), ({r, x}) => {
    let olddims = uop.shape(x)
    let shape = (r as UOpKind<"RESHAPE">).shape
  ...
  [new UPat("p", "PERMUTE", [new UPat("x")]), ({p, x}) => {
    ...
    (p as UOpKind<"PERMUTE">).shape.map((pd,i)=>{
      dims[i] = olddims[pd]
  ...
  [new UPat("e", "EXPAND", [new UPat("x")]), ({e, x}) => {
    let olddims = uop.shape(x)
    let oldstrides = stridesFor(olddims)
    let dims = (e as UOpKind<"EXPAND">).shape.slice()
    let strides = dims.map(_=>0)
  ...
  [new UPat("v1", "VIEW", [new UPat("v2", "VIEW")]), ({v1,v2})=>(mkUop("VIEW", [v2.srcs[0]!], {views: ([v1,v2] as UOpKind<"VIEW">[]).map(v=>v.views).flat()}))],
